...
set confirm off
set arch riscv:rv64
set logging file .build/qemu-virt/irq-pipeline.cpu1-badstack.gdb.log
...
break riscv_v_context_nesting_end
commands
  silent
  if $ttwu_hits > 0 && $tp != $init_current
    set $vnest_hits = $vnest_hits + 1
    if $vnest_hits <= 24
      printf "\n=== riscv_v_context_nesting_end hit %d ===\n", $vnest_hits
