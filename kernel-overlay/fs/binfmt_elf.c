...
static void elf_bootdbg_irq_context_fix(const char *where,
					unsigned long addr,
					unsigned long size)
{
	...
	pr_notice("BOOTDBG elf_irq_fix before where=%s addr=%lx size=%lx stall=%d irqs_disabled=%d hard_irqs_disabled=%d current=%s[%d]\n",
		  where, addr, size, test_inband_stall(), irqs_disabled(),
	...
	pr_notice("BOOTDBG elf_irq_fix after where=%s addr=%lx size=%lx stall=%d irqs_disabled=%d hard_irqs_disabled=%d current=%s[%d]\n",
		  where, addr, size, test_inband_stall(), irqs_disabled(),
		  hard_irqs_disabled(), current->comm, task_pid_nr(current));
}
#else
static inline void elf_bootdbg_irq_context_fix(const char *where,
					       unsigned long addr,
					       unsigned long size)
{
}
		...
		unsigned long total_size)
{
	...
	*/
	if (total_size) {
		...
		elf_bootdbg_irq_context_fix("elf_map_total_vm_mmap", addr,
					    total_size);
		map_addr = vm_mmap(filep, addr, total_size, prot, type, off);
		if (!BAD_ADDR(map_addr)) {
			elf_bootdbg_irq_context_fix("elf_map_vm_munmap",
						    map_addr + size,
						    total_size - size);
			vm_munmap(map_addr+size, total_size-size);
		}
	} else {
		elf_bootdbg_irq_context_fix("elf_map_vm_mmap", addr, size);
