...
static void open_bootdbg_irq_context_fix(const char *where)
{
	...
	pr_notice("BOOTDBG open_irq_fix before where=%s stall=%d irqs_disabled=%d hard_irqs_disabled=%d current=%s[%d]\n",
		  where, test_inband_stall(), irqs_disabled(),
	...
	pr_notice("BOOTDBG open_irq_fix after where=%s stall=%d irqs_disabled=%d hard_irqs_disabled=%d current=%s[%d]\n",
		  where, test_inband_stall(), irqs_disabled(),
		  hard_irqs_disabled(), current->comm, task_pid_nr(current));
}
#else
static inline void open_bootdbg_irq_context_fix(const char *where)
{
}
#endif
...
retry:
	...
	open_bootdbg_irq_context_fix("chdir_before_set_fs_pwd");
	set_fs_pwd(current->fs, &path);

dput_and_out:
	open_bootdbg_irq_context_fix("chdir_before_path_put");
	path_put(&path);
	if (retry_estale(error, lookup_flags)) {
		lookup_flags |= LOOKUP_REVAL;
		goto retry;
...
SYSCALL_DEFINE1(fchdir, unsigned int, fd)
{
	...
	if (!error) {
		open_bootdbg_irq_context_fix("fchdir_before_set_fs_pwd");
		set_fs_pwd(current->fs, &f.file->f_path);
	}
out_putf:
	open_bootdbg_irq_context_fix("fchdir_before_fdput");
	fdput(f);
out:
	return error;
}
