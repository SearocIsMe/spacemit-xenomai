...
static void genpd_queue_power_off_work(struct generic_pm_domain *genpd)
{
	...
	pr_info("BOOTDBG genpd_queue_power_off_work before_queue domain=%s status=%d device_count=%u suspended_count=%d prepared_count=%d work_pending=%d current=%s[%d] irqs_disabled=%d in_atomic=%d\n",
		genpd->name, genpd->status, genpd->device_count,
		genpd->suspended_count, genpd->prepared_count,
		work_pending(&genpd->power_off_work), current->comm, current->pid,
		irqs_disabled(), in_atomic());
	queued = queue_work(pm_wq, &genpd->power_off_work);
	pr_info("BOOTDBG genpd_queue_power_off_work after_queue domain=%s queued=%d status=%d device_count=%u suspended_count=%d prepared_count=%d work_pending=%d current=%s[%d] irqs_disabled=%d in_atomic=%d\n",
		genpd->name, queued, genpd->status, genpd->device_count,
		genpd->suspended_count, genpd->prepared_count,
		work_pending(&genpd->power_off_work), current->comm, current->pid,
		irqs_disabled(), in_atomic());
}
...
static void genpd_power_off_work_fn(struct work_struct *work)
{
	...
	pr_info("BOOTDBG genpd_power_off_work_fn enter domain=%s status=%d device_count=%u suspended_count=%d prepared_count=%d\n",
		genpd->name, genpd->status, genpd->device_count,
		genpd->suspended_count, genpd->prepared_count);
	...
	pr_info("BOOTDBG genpd_power_off_work_fn exit domain=%s status=%d device_count=%u suspended_count=%d prepared_count=%d\n",
		genpd->name, genpd->status, genpd->device_count,
		genpd->suspended_count, genpd->prepared_count);
}
...
static int __init genpd_power_off_unused(void)
{
	...
	pr_warn("BOOTDBG genpd: skipping unused power-domain power-off work during EVL boot debug\n");
	return 0;
...
static void genpd_dev_pm_sync(struct device *dev)
{
	...
	if (!strcmp(name, "d4000000.pdma") ||
	    !strcmp(name, "d4011000.i2c") ||
	    !strcmp(name, "d4012000.i2c") ||
	    !strcmp(name, "d401d800.i2c") ||
	    !strcmp(name, "c0887000.ri2c")) {
		debug_device = true;
		skip_for_debug = true;
		pr_info("BOOTDBG genpd_dev_pm_sync enter dev=%s domain=%s status=%d device_count=%u suspended_count=%d prepared_count=%d\n",
			name, pd->name, pd->status, pd->device_count,
			pd->suspended_count, pd->prepared_count);
	}

	if (skip_for_debug) {
		pr_info("BOOTDBG genpd_dev_pm_sync skipping_queue_work_for_debug dev=%s domain=%s\n",
			name, pd->name);
		return;
	}
	...
	if (debug_device)
		pr_info("BOOTDBG genpd_dev_pm_sync exit dev=%s domain=%s status=%d device_count=%u suspended_count=%d prepared_count=%d\n",
			name, pd->name, pd->status, pd->device_count,
			pd->suspended_count, pd->prepared_count);
}
