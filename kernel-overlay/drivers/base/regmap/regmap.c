...
static bool regmap_bootdbg_is_pmic_target(struct regmap *map)
{
	return map->dev && !strcmp(dev_name(map->dev), "8-0041");
}
		  ...
		  unsigned int val)
{
	...
	if (regmap_bootdbg_is_pmic_target(map))
		dev_info(map->dev,
			 "BOOTDBG _regmap_write before_hw reg=0x%x val=0x%x cache_bypass=%d defer_caching=%d\n",
			 reg, val, map->cache_bypass, map->defer_caching);
	...
	if (regmap_bootdbg_is_pmic_target(map))
		dev_info(map->dev,
			 "BOOTDBG _regmap_write after_hw reg=0x%x val=0x%x ret=%d\n",
			 reg, val, ret);
			...
			unsigned int *val)
{
	...
	if (regmap_bootdbg_is_pmic_target(map))
		dev_info(map->dev,
			 "BOOTDBG _regmap_read before_hw reg=0x%x cache_bypass=%d cache_only=%d\n",
			 reg, map->cache_bypass, map->cache_only);
	...
	if (regmap_bootdbg_is_pmic_target(map))
		dev_info(map->dev,
			 "BOOTDBG _regmap_read after_hw reg=0x%x ret=%d val=0x%x\n",
			 reg, ret, ret ? 0 : *val);
			       ...
			       bool *change, bool force_write)
{
	...
	if (regmap_volatile(map, reg) && map->reg_update_bits) {
		...
		if (regmap_bootdbg_is_pmic_target(map))
			dev_info(map->dev,
				 "BOOTDBG _regmap_update_bits before_read reg=0x%x mask=0x%x val=0x%x force=%d\n",
				 reg, mask, val, force_write);
		...
		if (regmap_bootdbg_is_pmic_target(map))
			dev_info(map->dev,
				 "BOOTDBG _regmap_update_bits after_read reg=0x%x orig=0x%x tmp=0x%x\n",
				 reg, orig, tmp);

		if (force_write || (tmp != orig) || map->force_write_field) {
			if (regmap_bootdbg_is_pmic_target(map))
				dev_info(map->dev,
					 "BOOTDBG _regmap_update_bits before_write reg=0x%x tmp=0x%x\n",
					 reg, tmp);
			...
			if (regmap_bootdbg_is_pmic_target(map))
				dev_info(map->dev,
					 "BOOTDBG _regmap_update_bits after_write reg=0x%x ret=%d change=%d\n",
					 reg, ret, change ? *change : -1);
		}
	}
