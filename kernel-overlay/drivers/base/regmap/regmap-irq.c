			       ...
			       struct regmap_irq_chip_data **data)
{
	...
	pr_info("BOOTDBG regmap_add_irq_chip_fwnode enter dev=%s irq=%d flags=0x%x base=%d chip=%s num_irqs=%d num_regs=%d status_base=0x%x mask_base=0x%x ack_base=0x%x\n",
		map && map->dev ? dev_name(map->dev) : "<none>",
		irq, irq_flags, irq_base,
		chip && chip->name ? chip->name : "<none>",
		chip ? chip->num_irqs : -1,
		chip ? chip->num_regs : -1,
		chip ? chip->status_base : 0,
		chip ? chip->mask_base : 0,
		chip ? chip->ack_base : 0);
	...
	d->chip = chip;
	d->irq_base = irq_base;
	pr_info("BOOTDBG regmap_add_irq_chip_fwnode after_alloc dev=%s d=%p irq=%d\n",
		map && map->dev ? dev_name(map->dev) : "<none>", d, irq);
	...
	pr_info("BOOTDBG regmap_add_irq_chip_fwnode after_mask_buf_def dev=%s\n",
		map && map->dev ? dev_name(map->dev) : "<none>");

	/* Mask all the interrupts by default */
	for (i = 0; i < chip->num_regs; i++) {
		d->mask_buf[i] = d->mask_buf_def[i];
		pr_info("BOOTDBG regmap_add_irq_chip_fwnode mask_loop_enter dev=%s idx=%d mask_def=0x%x mask=0x%x\n",
			map && map->dev ? dev_name(map->dev) : "<none>",
			i, d->mask_buf_def[i], d->mask_buf[i]);
		...
		if (chip->mask_base && !chip->handle_mask_sync) {
			...
			pr_info("BOOTDBG regmap_add_irq_chip_fwnode before_mask_update dev=%s idx=%d reg=0x%x val=0x%x\n",
				map && map->dev ? dev_name(map->dev) : "<none>",
				i, reg, d->mask_buf[i]);
			ret = regmap_update_bits(d->map, reg,
						 d->mask_buf_def[i],
			...
			pr_info("BOOTDBG regmap_add_irq_chip_fwnode after_mask_update dev=%s idx=%d reg=0x%x\n",
				map && map->dev ? dev_name(map->dev) : "<none>",
				i, reg);
		}
		...
		if (chip->unmask_base && !chip->handle_mask_sync) {
			...
			pr_info("BOOTDBG regmap_add_irq_chip_fwnode before_unmask_update dev=%s idx=%d reg=0x%x val=0x%x\n",
				map && map->dev ? dev_name(map->dev) : "<none>",
				i, reg, ~d->mask_buf[i]);
			ret = regmap_update_bits(d->map, reg,
					d->mask_buf_def[i], ~d->mask_buf[i]);
			...
			pr_info("BOOTDBG regmap_add_irq_chip_fwnode after_unmask_update dev=%s idx=%d reg=0x%x\n",
				map && map->dev ? dev_name(map->dev) : "<none>",
				i, reg);
		}
		...
		if (d->chip->no_status) {
			...
			reg = d->get_irq_reg(d, d->chip->status_base, i);
			pr_info("BOOTDBG regmap_add_irq_chip_fwnode before_status_read dev=%s idx=%d reg=0x%x\n",
				map && map->dev ? dev_name(map->dev) : "<none>",
				i, reg);
			...
			pr_info("BOOTDBG regmap_add_irq_chip_fwnode after_status_read dev=%s idx=%d reg=0x%x status=0x%x\n",
				map && map->dev ? dev_name(map->dev) : "<none>",
				i, reg, d->status_buf[i]);
		}
		...
		if (d->status_buf[i] && (chip->ack_base || chip->use_ack)) {
			reg = d->get_irq_reg(d, d->chip->ack_base, i);
			pr_info("BOOTDBG regmap_add_irq_chip_fwnode before_ack dev=%s idx=%d reg=0x%x status=0x%x mask=0x%x\n",
				map && map->dev ? dev_name(map->dev) : "<none>",
				i, reg, d->status_buf[i], d->mask_buf[i]);
			if (chip->ack_invert)
				ret = regmap_write(map, reg,
			...
			pr_info("BOOTDBG regmap_add_irq_chip_fwnode after_ack dev=%s idx=%d reg=0x%x\n",
				map && map->dev ? dev_name(map->dev) : "<none>",
				i, reg);
		}
	}

	pr_info("BOOTDBG regmap_add_irq_chip_fwnode after_mask_init dev=%s\n",
		map && map->dev ? dev_name(map->dev) : "<none>");
	...
	pr_info("BOOTDBG regmap_add_irq_chip_fwnode after_create_domain dev=%s domain=%p\n",
		map && map->dev ? dev_name(map->dev) : "<none>", d->domain);

	pr_info("BOOTDBG regmap_add_irq_chip_fwnode before_request_threaded_irq dev=%s irq=%d flags=0x%x\n",
		map && map->dev ? dev_name(map->dev) : "<none>",
		irq, irq_flags | IRQF_ONESHOT);
	...
	pr_info("BOOTDBG regmap_add_irq_chip_fwnode after_request_threaded_irq dev=%s irq=%d\n",
		map && map->dev ? dev_name(map->dev) : "<none>", irq);

	*data = d;
	pr_info("BOOTDBG regmap_add_irq_chip_fwnode success dev=%s irq=%d data=%p\n",
		map && map->dev ? dev_name(map->dev) : "<none>", irq, d);
