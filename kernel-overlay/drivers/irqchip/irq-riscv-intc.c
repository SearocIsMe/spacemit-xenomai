...
static void riscv_intc_irq_unmask(struct irq_data *d)
{
	...
	if (riscv_evl_trace_enabled() && current && task_cpu(current) == 1 &&
		...
		riscv_evl_trace_ulong("EVLDBG riscv_intc_irq_unmask hwirq=",
				      d->hwirq);
		riscv_evl_trace_task_stack_state(
			"EVLDBG riscv_intc_irq_unmask before",
...
static struct irq_chip riscv_intc_chip = {
	...
	/*
	 * IRQCHIP_PIPELINE_SAFE is required when CONFIG_IRQ_PIPELINE=y.
	 * Without it, kernel/irq/chip.c:irq_set_chip() fires a WARN_ONCE
	 * for every IRQ that uses this chip (timer, IPI, etc.), and the
	 * pipeline treats the chip as unsafe, which can prevent OOB
	 * interrupt delivery and stall the inband tick replay.
	 */
