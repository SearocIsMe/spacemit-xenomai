...
void bootdbg_plic_force_unmask_external_irq(unsigned int virq, const char *tag)
{
	struct irq_desc *desc;
	struct irq_data *data;
	...
	desc = irq_to_desc(virq);
	if (!desc) {
		pr_info("BOOTDBG plic_force_unmask tag=%s virq=%u desc=NULL\n",
			tag, virq);
		return;
	}

	data = irq_desc_get_irq_data(desc);
	if (!bootdbg_plic_trace_this_irq(data))
		return;

	pr_info("BOOTDBG plic_force_unmask tag=%s virq=%u hwirq=%lu\n",
		tag, data->irq, data->hwirq);
	plic_irq_unmask(data);
	bootdbg_plic_dump_irq_state(data, tag);
}
...
#ifdef CONFIG_SOC_SPACEMIT
	.flags		= IRQCHIP_AFFINITY_PRE_STARTUP | IRQCHIP_ENABLE_WAKEUP_ON_SUSPEND | IRQCHIP_SKIP_SET_WAKE | IRQCHIP_PIPELINE_SAFE,
#else
...
static int plic_starting_cpu(unsigned int cpu)
{
	...
	if (riscv_evl_trace_enabled())
		riscv_evl_trace_ulong("EVLDBG plic_starting_cpu entry cpu=", cpu);
#endif
	if (plic_parent_irq)
	...
	if (riscv_evl_trace_enabled()) {
		...
		riscv_evl_trace_ulong("EVLDBG plic_starting_cpu after threshold cpu=", cpu);
	}
#endif
