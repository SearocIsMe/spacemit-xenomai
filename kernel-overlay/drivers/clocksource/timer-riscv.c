		...
		struct clock_event_device *ce)
{
	...
	static unsigned int trace_next_event_count;
#endif

#ifdef CONFIG_IRQ_PIPELINE
	if (trace_next_event_count < 8) {
		trace_next_event_count++;
		riscv_evl_trace_ulong("EVLDBG riscv_clock_next_event delta=",
				      delta);
		riscv_evl_trace_ulong("EVLDBG riscv_clock_next_event next_tval=",
				      (unsigned long)next_tval);
		riscv_evl_trace_ulong("EVLDBG riscv_clock_next_event handler=",
...
#ifdef CONFIG_IRQ_PIPELINE
	static unsigned int trace_timer_irq_count;
#endif

#ifdef CONFIG_IRQ_PIPELINE
	if (trace_timer_irq_count < 16) {
		trace_timer_irq_count++;
		riscv_evl_trace_ulong("EVLDBG riscv_timer_interrupt irq=", irq);
		riscv_evl_trace_ulong("EVLDBG riscv_timer_interrupt handler=",
				      (unsigned long)evdev->event_handler);
	}
#endif
	csr_clear(CSR_IE, IE_TIE);
