...
static int spacemit_regulator_probe(struct platform_device *pdev)
{
	...
	pr_info("BOOTDBG spacemit_regulator_probe enter dev=%s parent=%s of_node=%s\n",
		dev_name(&pdev->dev),
		pdev->dev.parent ? dev_name(pdev->dev.parent) : "<none>",
		pdev->dev.of_node ? pdev->dev.of_node->name : "<none>");
	...
	pr_info("BOOTDBG spacemit_regulator_probe matched name=%s nr_desc=%d\n",
		match_data->name, match_data->nr_desc);
	...
	pr_info("BOOTDBG spacemit_regulator_probe config client=%s addr=0x%02x regmap=%p\n",
		dev_name(&client->dev), client->addr, config.regmap);

	for (i = 0; i < match_data->nr_desc; ++i) {
		pr_info("BOOTDBG spacemit_regulator_probe before_register idx=%d name=%s of_match=%s\n",
			i,
			match_data->desc[i].name ? match_data->desc[i].name : "<none>",
			match_data->desc[i].of_match ? match_data->desc[i].of_match : "<none>");
		regulator_dev = devm_regulator_register(&pdev->dev,
				match_data->desc + i, &config);
		if (IS_ERR(regulator_dev)) {
			pr_err("BOOTDBG spacemit_regulator_probe failed_register idx=%d name=%s ret=%ld\n",
			       i,
			       match_data->desc[i].name ? match_data->desc[i].name : "<none>",
			       PTR_ERR(regulator_dev));
			return PTR_ERR(regulator_dev);
		}
		pr_info("BOOTDBG spacemit_regulator_probe after_register idx=%d name=%s rdev=%p\n",
			i,
			match_data->desc[i].name ? match_data->desc[i].name : "<none>",
			regulator_dev);
	}

	pr_info("BOOTDBG spacemit_regulator_probe success dev=%s\n", dev_name(&pdev->dev));
	return 0;
}
