...
static void bootdbg_spacemit_i2c_dump_irq_desc(struct spacemit_i2c_dev *spacemit_i2c,
					       const char *tag)
{
	...
	desc = irq_to_desc(spacemit_i2c->irq);
	if (!desc) {
		dev_info(spacemit_i2c->dev,
			 "BOOTDBG spacemit_i2c_irq_desc tag=%s irq=%d desc=NULL\n",
			 tag, spacemit_i2c->irq);
		return;
	}
...
spacemit_i2c_xfer(struct i2c_adapter *adapt, struct i2c_msg msgs[], int num)
{
	...
	if (bootdbg_target)
		dev_info(spacemit_i2c->dev,
			 "BOOTDBG spacemit_i2c_xfer enter adap=%d num=%d addr0=0x%02x flags0=0x%x len0=%d irqs_disabled=%d hard_irqs_disabled=%d\n",
			 adapt->nr, num, msgs[0].addr, msgs[0].flags, msgs[0].len,
...
xfer_retry:
	...
	if (bootdbg_target)
		dev_info(spacemit_i2c->dev,
			 "BOOTDBG spacemit_i2c_xfer before_recover_bus_busy adap=%d mode=%d\n",
			 adapt->nr, spacemit_i2c->xfer_mode);
	...
	if (bootdbg_target)
		dev_info(spacemit_i2c->dev,
			 "BOOTDBG spacemit_i2c_xfer after_recover_bus_busy adap=%d ret=%d\n",
			 adapt->nr, ret);

	/* i2c msg transmit */
	if (bootdbg_target)
		dev_info(spacemit_i2c->dev,
			 "BOOTDBG spacemit_i2c_xfer before_msg_transmit adap=%d mode=%d\n",
			 adapt->nr, spacemit_i2c->xfer_mode);
	if (likely(spacemit_i2c->xfer_mode == SPACEMIT_I2C_MODE_INTERRUPT))
		ret = spacemit_i2c_byte_xfer(spacemit_i2c);
	else if (likely(spacemit_i2c->xfer_mode == SPACEMIT_I2C_MODE_FIFO))
	...
	if (bootdbg_target)
		dev_info(spacemit_i2c->dev,
			 "BOOTDBG spacemit_i2c_xfer after_msg_transmit adap=%d mode=%d ret=%d num=%d phase=%d status=0x%x err=0x%x\n",
...
timeout_xfex:
	...
	if (bootdbg_target)
		dev_info(spacemit_i2c->dev,
			 "BOOTDBG spacemit_i2c_xfer timeout_or_exit adap=%d ret=%d status=0x%x err=0x%x num=%d\n",
			 adapt->nr, ret, spacemit_i2c->i2c_status,
			 spacemit_i2c->i2c_err, spacemit_i2c->num);
...
static int spacemit_i2c_probe(struct platform_device *pdev)
{
	...
	pr_info("BOOTDBG spacemit_i2c_probe enter dev=%s of_node=%s\n",
		dev_name(&pdev->dev), dnode ? dnode->full_name : "<none>");
	...
	pr_info("BOOTDBG spacemit_i2c_probe after_devm_kzalloc spacemit_i2c=%px\n",
		spacemit_i2c);

	spacemit_i2c->dev = &pdev->dev;
	platform_set_drvdata(pdev, spacemit_i2c);
	mutex_init(&spacemit_i2c->mtx);
	pr_info("BOOTDBG spacemit_i2c_probe after_platform_set_drvdata\n");
	...
	pr_info("BOOTDBG spacemit_i2c_probe after_get_optional_reset resets=%px\n",
		spacemit_i2c->resets);
	/* reset the i2c controller */
	pr_info("BOOTDBG spacemit_i2c_probe before_reset_toggle\n");
	reset_control_assert(spacemit_i2c->resets);
	udelay(200);
	reset_control_deassert(spacemit_i2c->resets);
	pr_info("BOOTDBG spacemit_i2c_probe after_reset_toggle\n");

	pr_info("BOOTDBG spacemit_i2c_probe before_parse_dt\n");
	ret = spacemit_i2c_parse_dt(pdev, spacemit_i2c);
	if (ret)
		goto err_out;
	pr_info("BOOTDBG spacemit_i2c_probe after_parse_dt ret=%d\n", ret);

	pr_info("BOOTDBG spacemit_i2c_probe before_of_address_to_resource\n");
	...
	pr_info("BOOTDBG spacemit_i2c_probe after_of_address_to_resource start=%pa end=%pa\n",
		&spacemit_i2c->resrc.start, &spacemit_i2c->resrc.end);

	pr_info("BOOTDBG spacemit_i2c_probe before_ioremap_resource\n");
	...
	pr_info("BOOTDBG spacemit_i2c_probe after_ioremap_resource mapbase=%px\n",
...
#endif
	{
		pr_info("BOOTDBG spacemit_i2c_probe before_platform_get_irq\n");
		...
		pr_info("BOOTDBG spacemit_i2c_probe after_platform_get_irq irq=%d\n",
			spacemit_i2c->irq);

		pr_info("BOOTDBG spacemit_i2c_probe before_devm_request_irq\n");
		...
		pr_info("BOOTDBG spacemit_i2c_probe after_devm_request_irq ret=%d\n", ret);
		bootdbg_spacemit_i2c_dump_irq_desc(spacemit_i2c, "after_request_irq");
		bootdbg_spacemit_i2c_dump_irqchip_state(spacemit_i2c, "after_request_irq");
	}

	pr_info("BOOTDBG spacemit_i2c_probe before_prepare_dma\n");
	ret = spacemit_i2c_prepare_dma(spacemit_i2c);
	if (ret) {
		dev_err(&pdev->dev, "failed to request dma channels\n");
		goto err_out;
	}
	pr_info("BOOTDBG spacemit_i2c_probe after_prepare_dma ret=%d dma_disable=%d\n",
		ret, spacemit_i2c->dma_disable);

	pr_info("BOOTDBG spacemit_i2c_probe before_devm_clk_get\n");
	...
	pr_info("BOOTDBG spacemit_i2c_probe after_devm_clk_get clk=%px\n",
		spacemit_i2c->clk);
#ifdef CONFIG_SOC_SPACEMIT_K1X
	if (rcpu_i2c) {
...
#endif
	pr_info("BOOTDBG spacemit_i2c_probe before_clk_prepare_enable\n");
	clk_prepare_enable(spacemit_i2c->clk);
	pr_info("BOOTDBG spacemit_i2c_probe after_clk_prepare_enable\n");
	...
	if (!spacemit_i2c->clk_always_on) {
		pr_info("BOOTDBG spacemit_i2c_probe before_pm_runtime_enable\n");
		...
		pr_info("BOOTDBG spacemit_i2c_probe after_pm_runtime_enable\n");
	} else
	...
	pr_info("BOOTDBG spacemit_i2c_probe before_i2c_add_numbered_adapter nr=%d\n",
		spacemit_i2c->adapt.nr);
	ret = i2c_add_numbered_adapter(&spacemit_i2c->adapt);
	if (ret) {
		dev_err(spacemit_i2c->dev, "failed to add i2c adapter\n");
		goto err_clk;
	}
	pr_info("BOOTDBG spacemit_i2c_probe after_i2c_add_numbered_adapter ret=%d\n", ret);
	...
	pr_info("BOOTDBG spacemit_i2c_probe after_debugfs_create_file dbgfs=%px\n",
		spacemit_i2c->dbgfs);
#endif

	pr_info("BOOTDBG spacemit_i2c_probe success dev=%s\n", dev_name(&pdev->dev));
...
static int __init spacemit_i2c_init(void)
{
	...
	pr_info("BOOTDBG spacemit_i2c_init before_register_restart_handler\n");
	register_restart_handler(&spacemit_i2c_sys_nb);
	pr_info("BOOTDBG spacemit_i2c_init after_register_restart_handler\n");

	pr_info("BOOTDBG spacemit_i2c_init before_register_sys_off_handler\n");
	...
	pr_info("BOOTDBG spacemit_i2c_init after_platform_driver_register ret=%d\n", ret);
