...
static int __init k1xisp_dev_driver_init(void)
{
	pr_info("BOOTDBG skipping K1X ISP device init for boot debug\n");
	return 0;
}

static void __exit k1xisp_dev_driver_exit(void)
{
	platform_driver_unregister(&k1xisp_dev_driver);
}

module_init(k1xisp_dev_driver_init);
module_exit(k1xisp_dev_driver_exit);
