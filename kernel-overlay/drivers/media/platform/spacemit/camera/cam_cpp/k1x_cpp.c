...
static int __init cpp_driver_init(void)
{
	pr_info("BOOTDBG skipping K1X CPP camera post-process init for boot debug\n");
	return 0;
}

static void __exit cpp_driver_exit(void)
{
	platform_driver_unregister(&cpp_driver);
}

module_init(cpp_driver_init);
module_exit(cpp_driver_exit);
