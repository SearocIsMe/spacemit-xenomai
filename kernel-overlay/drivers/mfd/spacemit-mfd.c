...
static int spacemit_prepare_sub_pmic(struct spacemit_pmic *pmic)
{
	...
	pr_info("BOOTDBG spacemit_prepare_sub_pmic enter client=%s addr=0x%02x\n",
		dev_name(&client->dev), client->addr);

	sub->power_page_addr = pmic->i2c->addr + 1;
	pr_info("BOOTDBG spacemit_prepare_sub_pmic power_page_addr=0x%02x\n",
		sub->power_page_addr);

	sub->power_page = i2c_new_dummy_device(client->adapter,
			sub->power_page_addr);
	if (sub->power_page == NULL) {
		pr_err("BOOTDBG spacemit_prepare_sub_pmic failed_new_dummy_device addr=0x%02x\n",
		       sub->power_page_addr);
		return -ENODEV;
	}

	pr_info("BOOTDBG spacemit_prepare_sub_pmic after_new_dummy_device dev=%s\n",
		dev_name(&sub->power_page->dev));

	sub->power_regmap = devm_regmap_init_i2c(sub->power_page,
			pmic->regmap_cfg);
	if (IS_ERR(sub->power_regmap)) {
		pr_err("BOOTDBG spacemit_prepare_sub_pmic failed_regmap ret=%ld\n",
		       PTR_ERR(sub->power_regmap));
	...
	pr_info("BOOTDBG spacemit_prepare_sub_pmic success dev=%s\n",
		dev_name(&sub->power_page->dev));
...
static int spacemit_pmic_probe(struct i2c_client *client)
{
	...
	pr_info("BOOTDBG spacemit_pmic_probe enter dev=%s irq=%d of_node=%s\n",
		dev_name(&client->dev), client->irq,
		client->dev.of_node ? client->dev.of_node->name : "<none>");
	...
	pr_info("BOOTDBG spacemit_pmic_probe matched name=%s nr_cells=%d\n",
		match_data->name, match_data->nr_cells);
	...
	pr_info("BOOTDBG spacemit_pmic_probe after_regmap_init dev=%s regmap=%p\n",
		dev_name(&client->dev), pmic->regmap);
	...
	for (ret = 0; ret < nr_cells; ret++)
		pr_info("BOOTDBG spacemit_pmic_probe cell[%d] name=%s of_compatible=%s\n",
			ret, cells[ret].name ? cells[ret].name : "<none>",
			cells[ret].of_compatible ? cells[ret].of_compatible : "<none>");

	pr_info("BOOTDBG spacemit_pmic_probe before_add_devices dev=%s nr_cells=%d\n",
		dev_name(&client->dev), nr_cells);
	...
	pr_info("BOOTDBG spacemit_pmic_probe after_add_devices dev=%s\n",
		dev_name(&client->dev));
	...
	pr_info("BOOTDBG spacemit_pmic_probe success dev=%s\n", dev_name(&client->dev));
	return 0;
}
