...
static int __init chr_dev_init(void)
{
	...
	pr_info("BOOTDBG mem_chr_dev_init enter\n");
	...
	pr_info("BOOTDBG mem_chr_dev_init after_class_register\n");
	for (minor = 1; minor < ARRAY_SIZE(devlist); minor++) {
		...
		pr_info("BOOTDBG mem_chr_dev_init device_create minor=%d name=%s\n",
			minor, devlist[minor].name);
		device_create(&mem_class, NULL, MKDEV(MEM_MAJOR, minor),
			      NULL, devlist[minor].name);
	}

	pr_info("BOOTDBG mem_chr_dev_init before_tty_init\n");
	retval = tty_init();
	pr_info("BOOTDBG mem_chr_dev_init after_tty_init ret=%d\n", retval);
	return retval;
}
