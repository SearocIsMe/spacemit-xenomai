...
int mmc_add_host(struct mmc_host *host)
{
	...
	pr_notice("BOOTDBG mmc_add_host %s enter caps=0x%x caps2=0x%x ocr=0x%x f_min=%u f_max=%u\n",
		  mmc_hostname(host), host->caps, host->caps2, host->ocr_avail,
		  host->f_min, host->f_max);
	err = mmc_validate_host_caps(host);
	pr_notice("BOOTDBG mmc_add_host %s after_validate ret=%d\n",
		  mmc_hostname(host), err);
	if (err)
		return err;

	pr_notice("BOOTDBG mmc_add_host %s before_device_add\n", mmc_hostname(host));
	err = device_add(&host->class_dev);
	pr_notice("BOOTDBG mmc_add_host %s after_device_add ret=%d\n",
		  mmc_hostname(host), err);
	if (err)
		return err;

	pr_notice("BOOTDBG mmc_add_host %s before_led_trigger\n", mmc_hostname(host));
	led_trigger_register_simple(dev_name(&host->class_dev), &host->led);

	pr_notice("BOOTDBG mmc_add_host %s before_debugfs\n", mmc_hostname(host));
	mmc_add_host_debugfs(host);

	pr_notice("BOOTDBG mmc_add_host %s before_mmc_start_host\n", mmc_hostname(host));
	mmc_start_host(host);
	pr_notice("BOOTDBG mmc_add_host %s after_mmc_start_host\n", mmc_hostname(host));
	return 0;
}
