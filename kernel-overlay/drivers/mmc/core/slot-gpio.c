...
void mmc_gpiod_request_cd_irq(struct mmc_host *host)
{
	...
	if (system_state < SYSTEM_RUNNING) {
		pr_notice("BOOTDBG mmc_gpiod_request_cd_irq %s boot_skip_cd_irq system_state=%d caps=0x%x\n",
			  mmc_hostname(host), system_state, host->caps);
		host->slot.cd_irq = -EAGAIN;
	...
	if (ctx->cd_irq >= 0)
		...
		pr_notice("BOOTDBG mmc_gpiod_request_cd_irq %s before_gpiod_to_irq\n",
			  mmc_hostname(host));
		irq = gpiod_to_irq(ctx->cd_gpio);
		pr_notice("BOOTDBG mmc_gpiod_request_cd_irq %s after_gpiod_to_irq irq=%d\n",
			  mmc_hostname(host), irq);
	}

	if (irq >= 0) {
		if (!ctx->cd_gpio_isr)
			ctx->cd_gpio_isr = mmc_gpio_cd_irqt;
		pr_notice("BOOTDBG mmc_gpiod_request_cd_irq %s before_request_threaded_irq irq=%d\n",
			  mmc_hostname(host), irq);
		ret = devm_request_threaded_irq(host->parent, irq,
			NULL, ctx->cd_gpio_isr,
			IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING | IRQF_ONESHOT,
			ctx->cd_label, host);
		pr_notice("BOOTDBG mmc_gpiod_request_cd_irq %s after_request_threaded_irq irq=%d ret=%d\n",
			  mmc_hostname(host), irq, ret);
		if (ret < 0)
			irq = ret;
	}
	...
	pr_notice("BOOTDBG mmc_gpiod_request_cd_irq %s exit irq=%d caps=0x%x\n",
		  mmc_hostname(host), irq, host->caps);
}
