				...
				unsigned short vdd)
{
	...
	pr_notice("BOOTDBG sdhci_set_power_reg %s mode=%u vdd=%u before_regulator\n",
		  mmc_hostname(mmc), mode, vdd);
	ret = mmc_regulator_set_ocr(mmc, mmc->supply.vmmc, vdd);
	pr_notice("BOOTDBG sdhci_set_power_reg %s mode=%u vdd=%u after_regulator ret=%d enabled=%d\n",
		  mmc_hostname(mmc), mode, vdd, ret, mmc->regulator_enabled);

	if (mode != MMC_POWER_OFF)
		sdhci_writeb(host, SDHCI_POWER_ON, SDHCI_POWER_CONTROL);
	else
		sdhci_writeb(host, 0, SDHCI_POWER_CONTROL);
	pr_notice("BOOTDBG sdhci_set_power_reg %s after_power_control mode=%u\n",
		  mmc_hostname(mmc), mode);
}
...
void sdhci_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	if (ios->power_mode == MMC_POWER_UNDEFINED)
		return;
	pr_notice("BOOTDBG sdhci_set_ios %s enter clock=%u host_clock=%u power=%u vdd=%u bus_width=%u timing=%u\n",
		  mmc_hostname(mmc), ios->clock, host->clock, ios->power_mode,
		  ios->vdd, ios->bus_width, ios->timing);
	...
	if (!ios->clock || ios->clock != host->clock) {
		...
		pr_notice("BOOTDBG sdhci_set_ios %s before_set_clock clock=%u\n",
			  mmc_hostname(mmc), ios->clock);
		host->ops->set_clock(host, ios->clock);
		host->clock = ios->clock;
		pr_notice("BOOTDBG sdhci_set_ios %s after_set_clock host_clock=%u actual=%u\n",
			  mmc_hostname(mmc), host->clock, mmc->actual_clock);
	...
	pr_notice("BOOTDBG sdhci_set_ios %s before_set_power power=%u vdd=%u\n",
		  mmc_hostname(mmc), ios->power_mode, ios->vdd);
	if (host->ops->set_power)
		host->ops->set_power(host, ios->power_mode, ios->vdd);
	else
		sdhci_set_power(host, ios->power_mode, ios->vdd);
	pr_notice("BOOTDBG sdhci_set_ios %s after_set_power power=%u vdd=%u\n",
		  mmc_hostname(mmc), ios->power_mode, ios->vdd);

	if (host->ops->platform_send_init_74_clocks) {
		pr_notice("BOOTDBG sdhci_set_ios %s before_init_74_clocks power=%u\n",
			  mmc_hostname(mmc), ios->power_mode);
		host->ops->platform_send_init_74_clocks(host, ios->power_mode);
		pr_notice("BOOTDBG sdhci_set_ios %s after_init_74_clocks power=%u\n",
			  mmc_hostname(mmc), ios->power_mode);
	}

	pr_notice("BOOTDBG sdhci_set_ios %s before_set_bus_width width=%u\n",
		  mmc_hostname(mmc), ios->bus_width);
	host->ops->set_bus_width(host, ios->bus_width);
	pr_notice("BOOTDBG sdhci_set_ios %s after_set_bus_width width=%u\n",
		  mmc_hostname(mmc), ios->bus_width);
	...
	if (!reinit_uhs &&
		...
		pr_notice("BOOTDBG sdhci_set_ios %s return_after_turning_on_clk\n",
			  mmc_hostname(mmc));
		return;
	}
	...
	if (host->version >= SDHCI_SPEC_300) {
		...
		if (!host->preset_enabled) {
			...
			pr_notice("BOOTDBG sdhci_set_ios %s before_set_uhs timing=%u\n",
				  mmc_hostname(mmc), ios->timing);
			host->ops->set_uhs_signaling(host, ios->timing);
			pr_notice("BOOTDBG sdhci_set_ios %s after_set_uhs timing=%u\n",
				  mmc_hostname(mmc), ios->timing);
			host->timing = ios->timing;
		...
		if (sdhci_preset_needed(host, ios->timing)) {
			...
			/* Re-enable SD Clock */
			pr_notice("BOOTDBG sdhci_set_ios %s before_reenable_clock clock=%u\n",
				  mmc_hostname(mmc), host->clock);
			host->ops->set_clock(host, host->clock);
			pr_notice("BOOTDBG sdhci_set_ios %s after_reenable_clock clock=%u\n",
				  mmc_hostname(mmc), host->clock);
		} else
			sdhci_writeb(host, ctrl, SDHCI_HOST_CONTROL);
	pr_notice("BOOTDBG sdhci_set_ios %s exit\n", mmc_hostname(mmc));
}
...
int __sdhci_add_host(struct sdhci_host *host)
{
	...
	pr_notice("BOOTDBG __sdhci_add_host %s enter irq=%d caps=0x%x caps2=0x%x quirks=0x%x quirks2=0x%x\n",
		  mmc_hostname(mmc), host->irq, mmc->caps, mmc->caps2,
	...
	if (!host->complete_wq)
		return -ENOMEM;
	pr_notice("BOOTDBG __sdhci_add_host %s after_alloc_workqueue\n",
		  mmc_hostname(mmc));
	...
	pr_notice("BOOTDBG __sdhci_add_host %s before_sdhci_init\n",
		  mmc_hostname(mmc));
	sdhci_init(host, 0);
	pr_notice("BOOTDBG __sdhci_add_host %s after_sdhci_init\n",
		  mmc_hostname(mmc));

	pr_notice("BOOTDBG __sdhci_add_host %s before_request_irq irq=%d\n",
		  mmc_hostname(mmc), host->irq);
	ret = request_threaded_irq(host->irq, sdhci_irq, sdhci_thread_irq,
				   IRQF_SHARED,	mmc_hostname(mmc), host);
	pr_notice("BOOTDBG __sdhci_add_host %s after_request_irq ret=%d\n",
		  mmc_hostname(mmc), ret);
	if (ret) {
	...
	pr_notice("BOOTDBG __sdhci_add_host %s before_led_register\n",
		  mmc_hostname(mmc));
	ret = sdhci_led_register(host);
	pr_notice("BOOTDBG __sdhci_add_host %s after_led_register ret=%d\n",
		  mmc_hostname(mmc), ret);
	if (ret) {
	...
	pr_notice("BOOTDBG __sdhci_add_host %s before_mmc_add_host\n",
		  mmc_hostname(mmc));
	ret = mmc_add_host(mmc);
	pr_notice("BOOTDBG __sdhci_add_host %s after_mmc_add_host ret=%d\n",
		  mmc_hostname(mmc), ret);
	if (ret)
		goto unled;
	...
	pr_notice("BOOTDBG __sdhci_add_host %s before_enable_card_detection\n",
		  mmc_hostname(mmc));
	sdhci_enable_card_detection(host);
	pr_notice("BOOTDBG __sdhci_add_host %s return_ok\n", mmc_hostname(mmc));
...
int sdhci_add_host(struct sdhci_host *host)
{
	...
	pr_notice("BOOTDBG sdhci_add_host %s before_setup_host\n",
		  mmc_hostname(host->mmc));
	ret = sdhci_setup_host(host);
	pr_notice("BOOTDBG sdhci_add_host %s after_setup_host ret=%d\n",
		  mmc_hostname(host->mmc), ret);
	if (ret)
		return ret;

	pr_notice("BOOTDBG sdhci_add_host %s before___sdhci_add_host\n",
		  mmc_hostname(host->mmc));
	ret = __sdhci_add_host(host);
	pr_notice("BOOTDBG sdhci_add_host %s after___sdhci_add_host ret=%d\n",
		  mmc_hostname(host->mmc), ret);
	if (ret)
		goto cleanup;
