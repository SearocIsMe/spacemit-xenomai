						...
						struct mmc_ios *ios)
{
	...
	pr_notice("BOOTDBG spacemit_sdhci_start_signal_voltage_switch %s enter signal_voltage=%u version=0x%x flags=0x%x\n",
		  mmc_hostname(mmc), ios->signal_voltage, host->version, host->flags);
	...
	if (host->version < SDHCI_SPEC_300) {
		pr_notice("BOOTDBG spacemit_sdhci_start_signal_voltage_switch %s old_spec_skip\n",
			  mmc_hostname(mmc));
		return 0;
	}
	...
	case MMC_SIGNAL_VOLTAGE_330:
		...
		if (!IS_ERR(mmc->supply.vqmmc)) {
			pr_notice("BOOTDBG spacemit_sdhci_start_signal_voltage_switch %s before_vqmmc_330\n",
				  mmc_hostname(mmc));
			ret = mmc_regulator_set_vqmmc(mmc, ios);
			pr_notice("BOOTDBG spacemit_sdhci_start_signal_voltage_switch %s after_vqmmc_330 ret=%d\n",
	...
	case MMC_SIGNAL_VOLTAGE_180:
		if (!(host->flags & SDHCI_SIGNALING_180))
			return -EINVAL;
		if (!IS_ERR(mmc->supply.vqmmc)) {
			pr_notice("BOOTDBG spacemit_sdhci_start_signal_voltage_switch %s before_vqmmc_180\n",
				  mmc_hostname(mmc));
			ret = mmc_regulator_set_vqmmc(mmc, ios);
			pr_notice("BOOTDBG spacemit_sdhci_start_signal_voltage_switch %s after_vqmmc_180 ret=%d\n",
				  mmc_hostname(mmc), ret);
			if (ret < 0) {
	...
	case MMC_SIGNAL_VOLTAGE_120:
		if (!(host->flags & SDHCI_SIGNALING_120))
			return -EINVAL;
		if (!IS_ERR(mmc->supply.vqmmc)) {
			pr_notice("BOOTDBG spacemit_sdhci_start_signal_voltage_switch %s before_vqmmc_120\n",
				  mmc_hostname(mmc));
			ret = mmc_regulator_set_vqmmc(mmc, ios);
			pr_notice("BOOTDBG spacemit_sdhci_start_signal_voltage_switch %s after_vqmmc_120 ret=%d\n",
				  mmc_hostname(mmc), ret);
			if (ret < 0) {
...
static int spacemit_sdhci_probe(struct platform_device *pdev)
{
	...
	dev_notice(dev, "BOOTDBG sdhci_probe enter\n");
	host = sdhci_pltfm_init(pdev, &sdhci_k1x_pdata, sizeof(*spacemit));
	if (IS_ERR(host)) {
		dev_notice(dev, "BOOTDBG sdhci_probe sdhci_pltfm_init ret=%ld\n",
			   PTR_ERR(host));
		return PTR_ERR(host);
	}
	dev_notice(dev, "BOOTDBG sdhci_probe after_pltfm_init host=%p mmc=%p irq=%d\n",
		   host, host->mmc, host->irq);

	pltfm_host = sdhci_priv(host);

	spacemit = sdhci_pltfm_priv(pltfm_host);

	dev_notice(dev, "BOOTDBG sdhci_probe before_clk_io\n");
	...
	dev_notice(dev, "BOOTDBG sdhci_probe before_clk_io_enable\n");
	clk_prepare_enable(spacemit->clk_io);
	dev_notice(dev, "BOOTDBG sdhci_probe after_clk_io_enable\n");

	spacemit->clk_core = devm_clk_get(dev, "sdh-core");
	if (!IS_ERR(spacemit->clk_core)) {
		dev_notice(dev, "BOOTDBG sdhci_probe before_clk_core_enable\n");
		clk_prepare_enable(spacemit->clk_core);
		dev_notice(dev, "BOOTDBG sdhci_probe after_clk_core_enable\n");
	}

	spacemit->clk_aib = devm_clk_get(dev, "aib-clk");
	if (!IS_ERR(spacemit->clk_aib)) {
		dev_notice(dev, "BOOTDBG sdhci_probe before_clk_aib_enable\n");
		clk_prepare_enable(spacemit->clk_aib);
		dev_notice(dev, "BOOTDBG sdhci_probe after_clk_aib_enable\n");
	}

	dev_notice(dev, "BOOTDBG sdhci_probe before_reset_get\n");
	...
	dev_notice(dev, "BOOTDBG sdhci_probe before_reset_deassert\n");
	ret = reset_control_deassert(spacemit->reset);
	if (ret)
		goto err_rst_get;
	dev_notice(dev, "BOOTDBG sdhci_probe after_reset_deassert\n");

	match = of_match_device(of_match_ptr(sdhci_spacemit_of_match), &pdev->dev);
	if (match) {
		dev_notice(dev, "BOOTDBG sdhci_probe before_mmc_of_parse\n");
		ret = mmc_of_parse(host->mmc);
		dev_notice(dev, "BOOTDBG sdhci_probe after_mmc_of_parse ret=%d caps=0x%x caps2=0x%x ocr=0x%x\n",
			   ret, host->mmc->caps, host->mmc->caps2,
			   host->mmc->ocr_avail);
		if (ret)
			goto err_of_parse;
		dev_notice(dev, "BOOTDBG sdhci_probe before_sdhci_get_of_property\n");
		sdhci_get_of_property(pdev);
		dev_notice(dev, "BOOTDBG sdhci_probe after_sdhci_get_of_property quirks=0x%x quirks2=0x%x\n",
	...
	dev_notice(dev, "BOOTDBG sdhci_probe before_spacemit_get_of_property\n");
	spacemit_get_of_property(host, dev, pdata);
	dev_notice(dev, "BOOTDBG sdhci_probe pdata freq=%u caps=0x%x caps2=0x%x caps_disable=0x%x caps2_disable=0x%x quirks=0x%x quirks2=0x%x\n",
		   pdata->host_freq, pdata->host_caps, pdata->host_caps2,
		   pdata->host_caps_disable, pdata->host_caps2_disable,
		   pdata->quirks, pdata->quirks2);
	if (pdata->quirks)
	...
	dev_notice(dev, "BOOTDBG sdhci_probe before_pm_runtime_get_sync\n");
	ret = pm_runtime_get_sync(&pdev->dev);
	dev_notice(dev, "BOOTDBG sdhci_probe after_pm_runtime_get_sync ret=%d\n", ret);

	/* set io clock rate */
	if (pdata->host_freq) {
		dev_notice(dev, "BOOTDBG sdhci_probe before_clk_set_rate freq=%u\n",
			   pdata->host_freq);
		ret = clk_set_rate(spacemit->clk_io, pdata->host_freq);
		dev_notice(dev, "BOOTDBG sdhci_probe after_clk_set_rate ret=%d\n", ret);
		if (ret) {
	...
	dev_notice(dev, "BOOTDBG sdhci_probe before_sdhci_add_host caps=0x%x caps2=0x%x ocr=0x%x f_min=%u f_max=%u power_delay=%u\n",
		   host->mmc->caps, host->mmc->caps2, host->mmc->ocr_avail,
		   host->mmc->f_min, host->mmc->f_max, host->mmc->ios.power_delay_ms);
	ret = sdhci_add_host(host);
	dev_notice(dev, "BOOTDBG sdhci_probe after_sdhci_add_host ret=%d\n", ret);
	if (ret) {
	...
	dev_notice(dev, "BOOTDBG sdhci_probe before_caps_disable\n");
	spacemit_sdhci_caps_disable(host);
	dev_notice(dev, "BOOTDBG sdhci_probe after_caps_disable caps=0x%x caps2=0x%x\n",
		   host->mmc->caps, host->mmc->caps2);

	if ((host->mmc->caps2 & MMC_CAP2_NO_MMC) || (host->quirks2 & SDHCI_QUIRK2_BROKEN_PHY_MODULE)) {
		pr_debug("%s: get card pinctrl\n", mmc_hostname(host->mmc));
		dev_notice(dev, "BOOTDBG sdhci_probe before_pinctrl_get\n");
		spacemit->pinctrl = devm_pinctrl_get(&pdev->dev);
		dev_notice(dev, "BOOTDBG sdhci_probe after_pinctrl_get ret=%ld\n",
			   IS_ERR(spacemit->pinctrl) ? PTR_ERR(spacemit->pinctrl) : 0L);
	}
	if (host->mmc->caps2 & MMC_CAP2_NO_MMC) {
#ifdef CONFIG_SYSFS
	...
	dev_notice(dev, "BOOTDBG sdhci_probe return_ok\n");
	return 0;
