...
int tty_register_driver(struct tty_driver *driver)
{
	...
	if (!(driver->flags & TTY_DRIVER_DYNAMIC_DEV)) {
		for (i = 0; i < driver->num; i++) {
			if (i < 4 || i >= 40 || i == driver->num - 1)
				pr_info("BOOTDBG tty_register_driver before_device name=%s index=%d num=%d major=%d minor_start=%d\n",
					driver->name, i, driver->num,
					driver->major, driver->minor_start);
			d = tty_register_device(driver, i, NULL);
			if (i < 4 || i >= 40 || i == driver->num - 1)
				pr_info("BOOTDBG tty_register_driver after_device name=%s index=%d dev=%px err=%ld\n",
					driver->name, i, d, IS_ERR(d) ? PTR_ERR(d) : 0L);
			if (IS_ERR(d)) {
				error = PTR_ERR(d);
				goto err_unreg_devs;
			}
...
{
	pr_info("BOOTDBG tty_init enter\n");
...
#ifdef CONFIG_VT
	pr_info("BOOTDBG tty_init before_vty_init\n");
	vty_init(&console_fops);
	pr_info("BOOTDBG tty_init after_vty_init\n");
#endif
	pr_info("BOOTDBG tty_init exit\n");
	return 0;
}
