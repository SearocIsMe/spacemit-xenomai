...
static inline irqreturn_t serial_pxa_irq(int irq, void *dev_id)
{
	...
	if (up->dma_enable) {
		...
		if (lsr & UART_LSR_DR) {
			pr_warn_ratelimited("BOOTDBG pxa_k1x_uart irq_rx line=%d irq=%d lsr=0x%x\n",
					    up->port.line, irq, lsr);
...
static void serial_pxa_rx_poll(struct timer_list *t)
{
	struct uart_pxa_port *up = from_timer(up, t, rx_poll_timer);
	unsigned int lsr;
	int status;

	if (!up->rx_poll_enabled || !up->port.state)
		return;
	...
	lsr = serial_in(up, UART_LSR);
	if (lsr & UART_LSR_DR) {
		pr_warn_ratelimited("BOOTDBG pxa_k1x_uart poll_rx line=%d irq=%d lsr=0x%x dma=%d\n",
				    up->port.line, up->port.irq, lsr,
				    up->dma_enable);
		status = lsr;
...
rearm:
	if (up->rx_poll_enabled)
		mod_timer(&up->rx_poll_timer,
			  jiffies + msecs_to_jiffies(PXA_RX_POLL_MS));
}
...
static int serial_pxa_startup(struct uart_port *port)
{
	...
	serial_out(up, UART_IER, up->ier);
	spin_unlock_irqrestore(&up->port.lock, flags);
	up->rx_poll_enabled = true;
	mod_timer(&up->rx_poll_timer,
		  jiffies + msecs_to_jiffies(PXA_RX_POLL_MS));
	pr_warn("BOOTDBG pxa_k1x_uart startup line=%d irq=%d ier=0x%x dma=%d rx_poll_ms=%d\n",
		up->port.line, up->port.irq, up->ier, up->dma_enable,
		PXA_RX_POLL_MS);
...
static int serial_pxa_probe(struct platform_device *dev)
{
	...
	timer_setup(&sport->rx_poll_timer, serial_pxa_rx_poll, 0);

	if (sport->port.line == 0 && sport->dma_enable) {
		dev_info(&dev->dev,
			 "BOOTDBG disabling DMA on console UART%d for RX polling triage\n",
			 sport->port.line);
		sport->dma_enable = 0;
	}
