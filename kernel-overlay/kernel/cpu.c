...
#ifdef CONFIG_IRQ_PIPELINE
	if (cpu <= 3) {
		riscv_evl_trace_ulong("EVLDBG bringup_wait_for_ap_online cpu=",
				      cpu);
		riscv_evl_trace_ulong("EVLDBG bringup_wait_for_ap_online target=",
				      st->target);
		riscv_evl_trace_ulong("EVLDBG bringup_wait_for_ap_online state=",
				      st->state);
	}
#endif
	return 0;
...
static int cpuhp_bringup_ap(unsigned int cpu)
{
...
#ifdef CONFIG_IRQ_PIPELINE
	if (cpu <= 3) {
		riscv_evl_trace_ulong("EVLDBG cpuhp_bringup_ap cpu=", cpu);
		riscv_evl_trace_ulong("EVLDBG cpuhp_bringup_ap target=",
				      st->target);
		riscv_evl_trace_ulong("EVLDBG cpuhp_bringup_ap state=",
				      st->state);
	}
#endif
	...
	{
		if (cpu <= 3)
			riscv_evl_trace_ulong("EVLDBG cpuhp_bringup_ap return_idle cpu=",
					      cpu);
		return 0;
	}
#else
...
#ifdef CONFIG_IRQ_PIPELINE
	if (cpu <= 3) {
		riscv_evl_trace_ulong("EVLDBG cpuhp_bringup_ap ret=", ret);
		riscv_evl_trace_ulong("EVLDBG cpuhp_bringup_ap final_state=",
				      st->state);
	}
#endif
	return ret;
...
#ifdef CONFIG_IRQ_PIPELINE
	if (cpu <= 3)
		riscv_evl_trace_ulong("EVLDBG cpuhp_bringup_ap fail=", ret);
#endif
	return ret;
}
#else
...
#ifdef CONFIG_IRQ_PIPELINE
	if (cpu >= 1 && cpu <= 3)
		riscv_evl_trace_ulong("EVLDBG cpuhp_up_callbacks entry state=", st->state);
#endif
	ret = cpuhp_invoke_callback_range(true, cpu, st, target);
#ifdef CONFIG_IRQ_PIPELINE
	if (cpu >= 1 && cpu <= 3)
		riscv_evl_trace_ulong("EVLDBG cpuhp_up_callbacks ret=", ret);
#endif
	if (ret) {
...
#ifdef CONFIG_IRQ_PIPELINE
	if (cpu >= 1 && cpu <= 3)
		riscv_evl_trace_cpuhp_state("EVLDBG cpuhp_thread_fun",
					    cpu, bringup, st->state,
					    st->target, st->should_run,
					    st->result);
	if (cpu >= 1 && cpu <= 3)
...
#ifdef CONFIG_IRQ_PIPELINE
	if (cpu >= 1 && cpu <= 3)
		riscv_evl_trace_cpuhp_state("EVLDBG cpuhp_thread_fun end",
					    cpu, bringup, st->state,
					    st->target, st->should_run,
					    st->result);
	if (cpu >= 1 && cpu <= 3)
...
#ifdef CONFIG_IRQ_PIPELINE
	if (cpu <= 3) {
		riscv_evl_trace_ulong("EVLDBG cpuhp_kick_ap_work cpu=", cpu);
		riscv_evl_trace_ulong("EVLDBG cpuhp_kick_ap_work ret=", ret);
		riscv_evl_trace_ulong("EVLDBG cpuhp_kick_ap_work prev_state=",
				      prev_state);
		riscv_evl_trace_ulong("EVLDBG cpuhp_kick_ap_work state=",
				      st->state);
		riscv_evl_trace_ulong("EVLDBG cpuhp_kick_ap_work target=",
				      st->target);
	}
#endif
...
#ifdef CONFIG_IRQ_PIPELINE
	if (cpu >= 1 && cpu <= 3)
		riscv_evl_trace_ulong("EVLDBG _cpu_up entry cpu=", cpu);
#endif
	cpus_write_lock();
...
#ifdef CONFIG_IRQ_PIPELINE
	if (cpu >= 1 && cpu <= 3)
		riscv_evl_trace_ulong("EVLDBG _cpu_up before set_state cpu=", cpu);
#endif
	cpuhp_set_state(cpu, st, target);
#ifdef CONFIG_IRQ_PIPELINE
	if (cpu >= 1 && cpu <= 3)
		riscv_evl_trace_ulong("EVLDBG _cpu_up after set_state state=", st->state);
#endif
	/*
	 * If the current CPU state is in the range of the AP hotplug thread,
...
#ifdef CONFIG_IRQ_PIPELINE
		if (cpu >= 1 && cpu <= 3)
			riscv_evl_trace_ulong("EVLDBG _cpu_up before kick_ap cpu=", cpu);
#endif
		ret = cpuhp_kick_ap_work(cpu);
#ifdef CONFIG_IRQ_PIPELINE
		if (cpu >= 1 && cpu <= 3)
			riscv_evl_trace_ulong("EVLDBG _cpu_up after kick_ap ret=", ret);
#endif
		/*
		 * The AP side has done the error rollback already. Just
		 * return the error code..
...
#ifdef CONFIG_IRQ_PIPELINE
	if (cpu >= 1 && cpu <= 3)
		riscv_evl_trace_ulong("EVLDBG _cpu_up after kick_block state=", st->state);
#endif
...
#ifdef CONFIG_IRQ_PIPELINE
	if (cpu >= 1 && cpu <= 3)
		riscv_evl_trace_ulong("EVLDBG _cpu_up before callbacks state=", st->state);
#endif
	ret = cpuhp_up_callbacks(cpu, st, target);
#ifdef CONFIG_IRQ_PIPELINE
	if (cpu >= 1 && cpu <= 3)
		riscv_evl_trace_ulong("EVLDBG _cpu_up after callbacks ret=", ret);
#endif
out:
#ifdef CONFIG_IRQ_PIPELINE
	if (cpu >= 1 && cpu <= 3) {
		riscv_evl_trace_ulong("EVLDBG _cpu_up cpu=", cpu);
		riscv_evl_trace_ulong("EVLDBG _cpu_up ret=", ret);
		riscv_evl_trace_ulong("EVLDBG _cpu_up state=", st->state);
		riscv_evl_trace_ulong("EVLDBG _cpu_up target=", target);
	}
#endif
	cpus_write_unlock();
	arch_smt_update();
...
#ifdef CONFIG_IRQ_PIPELINE
	if (cpu >= 1 && cpu <= 3) {
		riscv_evl_trace_ulong("EVLDBG cpu_up entry cpu=", cpu);
		riscv_evl_trace_ulong("EVLDBG cpu_up entry target=", target);
	}
#endif
	if (!cpu_possible(cpu)) {
...
#ifdef CONFIG_IRQ_PIPELINE
	if (cpu >= 1 && cpu <= 3) {
		riscv_evl_trace_ulong("EVLDBG cpu_up cpu=", cpu);
		riscv_evl_trace_ulong("EVLDBG cpu_up err=", err);
		riscv_evl_trace_ulong("EVLDBG cpu_up target=", target);
	}
#endif
	cpu_maps_update_done();
	return err;
...
#ifdef CONFIG_IRQ_PIPELINE
	riscv_evl_trace_ulong("EVLDBG cpuhp_bringup_mask target=", target);
#endif
...
#ifdef CONFIG_IRQ_PIPELINE
		if (cpu <= 3)
			riscv_evl_trace_ulong("EVLDBG cpuhp_bringup_mask cpu=", cpu);
#endif

#ifdef CONFIG_IRQ_PIPELINE
		if (cpu <= 3)
			riscv_evl_trace_ulong("EVLDBG cpuhp_bringup_mask before cpu_up=", cpu);
#endif
		ret = cpu_up(cpu, target);
#ifdef CONFIG_IRQ_PIPELINE
		if (cpu <= 3)
			riscv_evl_trace_ulong("EVLDBG cpuhp_bringup_mask cpu_ret=", ret);
#endif
...
#ifdef CONFIG_IRQ_PIPELINE
	riscv_evl_trace("EVLDBG cpuhp_bringup_mask done\n");
#endif
}
...
#ifdef CONFIG_IRQ_PIPELINE
	riscv_evl_trace("EVLDBG bringup_nonboot_cpus done\n");
#endif
}
